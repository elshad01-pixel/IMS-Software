...
const packageTierOrder: TenantPackageTier[] = ['ASSURANCE', 'CORE_IMS', 'QHSE_PRO'];

const minimumPackageTierByModule = packageTierOrder.reduce((accumulator, tier) => {
  for (const scope of Object.keys(packageScopeModules[tier]) as TenantScope[]) {
    for (const moduleKey of packageScopeModules[tier][scope]) {
      if (!accumulator[moduleKey]) {
        accumulator[moduleKey] = tier;
      }
    }
  }
  return accumulator;
